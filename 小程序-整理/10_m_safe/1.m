frame(fp_executionPath,fp_values,family,hostName,portNumber,unixSocketName,MaxListen,fd,err,maxconn,one,ret,service,hint,listen_index,added,addr_ai_family,addr,MAXADDR,ListenSocket_OF_listen_index,sock_path,addrs,rnode,istemp,firstDelBlock,A,RELEASE,bufHdr,bufHdr_tag_blockNum,bufHdr_tag_rnode,bufHdr_tag_rnode_spcNode,bufHdr_tag_rnode_dbNode,bufHdr_tag_rnode_relNode,bufHdr_flags,bufHdr_cntxDirty,LocalRefCount_i,LocalBufferDescriptors_i,NLocBuffer,i,NBuffers,bufHdr_refcount,j) and (
...
int bufHdr_refcount and skip;
 ...
 function RelFileNodeEquals ( int a,int b,int RValue )
 {
  ...
  function main (  )
 {
     frame(main_1_2_temp1,main_6_temp2,goto, temp3) and (
     int goto<==0 and skip;
     //istemp:=rand(RValue);
	 if(!feof(fp_values))then{
     ...
     if(goto=0)   then 
     {
         ...
         while(j<=NBuffers)
         {
             ...
             if(main_6_temp2) then 
             {
                  fputs("[temp2]#",fp_executionPath) and skip;
                 if(bufHdr_tag_blockNum>=firstDelBlock) then 
                 {
                      fputs("[bufHdr_tag_blockNum>=firstDelBlock]#",fp_executionPath) and skip;
