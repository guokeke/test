...
void main()
{
	...
	*/
	...
	apr_pool_create_ex((int*)&pchild, (int)pconf, NULL, (int)allocator);
	...
	ap_reopen_scoreboard((int)pchild, NULL, 0);
	status = apr_proc_mutex_child_init((int*)&accept_mutex, ap_lock_fname, (int)pchild);
	if (status != APR_SUCCESS) {
		ap_log_error(APLOG_MARK, APLOG_EMERG, status, (int)ap_server_conf,
	...
	ap_update_child_status((int)sbh, SERVER_READY, NULL);
	...
	while (!die_now) {
		...
		*/
		...
		(void)ap_update_child_status((int)sbh, SERVER_READY, NULL);
				...
				if (status != APR_SUCCESS) {
					...
					*/
					ap_log_error5(APLOG_MARK, APLOG_ERR, status,
						(int)ap_server_conf, (int)"apr_pollset_poll: (listen)");
					clean_child_exit(1);
				}
		...
		*/
		int rand_x = rand();
		if (ap_mpm_pod_check((int)pod) == APR_SUCCESS) { /* selected as idle? */
			die_now = 1;
		}
