...
int do_ACCEPT and skip;
 ...
 function ap_mpm_pod_check ( int a,int RValue )
 {
  ...
  function main ( )
 {
     frame(main_3_pfd,main_4_csd,main_4_temp$_1,main_4_8_9_numdesc,main_4_8_9_pdesc,main_4_temp$_2,return,break$,continue) and (
    // int RValue  and skip;
     ...
     while( return=0 AND   break$=0 AND   !die_now)
     {
         ...
         while( return=0 AND  1)
         {
             ...
             temp4:=APR_STATUS_IS_EINTR(status,RValue);
             if(status!=7) then 
             {
                  fputs("[status!=7]#",fp_executionPath) and skip;
                 if(temp4!=0) then 
                 {
                      fputs("[temp4!=0]#",fp_executionPath) and skip;
	...
	if(!feof(fp_values)) then{
	    ...
	    status:=rand(RValue)
         ...
         if(continue=0)  then 
         {
             //current_conn:=rand(RValue);
	if(!feof(fp_values)) then{
	    fscanf(fp_values, "%d", &current_conn) and skip
	 }
	 else{
	    current_conn:=rand(RValue)
	 };
             ...
             //temp2:=rand(RValue);
	if(!feof(fp_values)) then{
	    fscanf(fp_values, "%d", &temp2) and skip
	 }
	 else{
	    temp2:=rand(RValue)
	 };
             ...
             //temp3:=rand(RValue);
	if(!feof(fp_values)) then{
	    fscanf(fp_values, "%d", &temp3) and skip
	 }
	 else{
	    temp3:=rand(RValue)
	 };
             ...
             if(temp2=7) then 
             {
                  fputs("[temp2==7]#",fp_executionPath) and skip;
                 die_now:=1
             }
             else
             {
                  fputs("[!(temp2==7)]#",fp_executionPath) and skip;
                 if(ap_my_generation!=temp3) then 
                 {
                      fputs("[ap_my_generation!=temp3]#",fp_executionPath) and skip;
                     die_now:=1
