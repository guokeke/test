frame(temp_i,temp_j,syf,bytesIn,bytesOut,verbosity,keepInputFiles,smallMode,testFailsExist,globalCrc,numFileNames,numFilesProcessed,opMode,srcMode,longestFileName,inName,outName,progName,progNameReally,outputHandleJustInCase,block,quadrant,zptr,szptr,ftab,ll16,ll4,tt,ll8,unzftab,last,origPtr,blockSize100k,workFactor,workDone,workLimit,blockRandomised,firstAttempt,nBlocksRandomised,inUse,nInUse,seqToUnseq,unseqToSeq,selector,selectorMtf,nMTF,mtfFreq,len$,limit,base,perm,minLens,code,rfreq,crc32Table,bsBuff,bsLive,bsStream,bsWriting,incs,rNums,dbglvl,spec_fd,seedi) and (
 ...
 char inName[100] and skip;
 char outName[100] and skip;
 char *progName and skip;
 char progNameReally[100] and skip;
  ...
  function spec_init ( int RValue )
 {
     ...
     while(spec_init_i<3)
     {
         ...
         spec_fd[spec_init_i].buf:=(unsigned char *)malloc(spec_init_limit+(100*100));
  ...
  function spec_random_load ( int fd,int RValue )
 {
     ...
     char spec_random_load_random_text[32,128*100] and skip;
     ...
     while(spec_random_load_i<32)
     {
         ...
         while(spec_random_load_j<(128*100))
         {
     ...
     while(spec_random_load_i<spec_fd[fd].limit)
     {
         ...
         memcpy(spec_fd[fd].buf+spec_random_load_i,spec_random_load_random_text[(int)(spec_random_load_temp$_1*32)],(128*100),RValue) and skip;
         spec_random_load_i:=spec_random_load_i+(128*100)
         
     };
     spec_fd[fd].len$:=100*100;
     return<==1 and RValue:=0;
     skip
     )
     }; 
  function spec_load ( int num,char *filename,int size,int RValue )
 {
     ...
     {
         spec_load_rc:=_read(spec_load_fd,spec_fd[num].buf+spec_load_i,(128*100),RValue);
 ...
 {
     frame(main_i,main_level,main_input_size,main_compressed_size,main_input_name,main_validate_array,return) and ( 
     int return<==0 and skip;
     int main_i,main_level and skip;
     int main_input_size<==64,main_compressed_size and skip;
     char main_input_name[22]<=="F:\\output.txt" and skip;
    ...
    /* if(argc>1) then 
	 ...
	 main_compressed_size:=main_input_size;
     spec_fd[0].limit:=main_input_size*(100*100);
     spec_fd[1].limit:=main_compressed_size*(100*100);
     spec_fd[2].limit:=main_input_size*(100*100);
     spec_init(RValue);
     debug_time(RValue);
     debug(2,"Loading Input Data\n");
     spec_load(0,main_input_name,main_input_size*(100*100),RValue);
     debug1(3,"Input data %d bytes in length\n",spec_fd[0].len$);
     main_validate_array:=(unsigned char *)malloc(main_input_size*(100*100)/ 100);
     ...
     while(main_i*1027<main_input_size*(100*100))
     {
