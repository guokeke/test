//#Unsafe
//
//Formula: (false R (! ((iE & ! oY) & (true U oY)) | (! oV U oY)))
//"output V does never occur between input E and output Y"
//Formula is not satisfied! An error path is
//[iE, oV, iF, oY, iA, oU, iB, oZ, iE, oY, iD, oV] ([iE, oZ])*
//Problem14.c
//(G (! ((iE & ! oY) & (F oY)) | (! oV U oY)));
#include <stdio.h> 
#include <assert.h>
#include <math.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! ((AP(input == 5) && ! AP(output == 25)) && (<> AP(output == 25))) || (! AP(output == 22) U AP(output == 25))));
