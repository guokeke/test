//#Unsafe
//
//Formula: (false R (! ((oV & ! iE) & (true U iE)) | (! oY U iE)))
//"output Y does never occur between output V and input E"
//Formula is not satisfied! An error path is
//[iE, oV, iF, oY, iA, oU, iB, oZ, iE, oY, iD, oV] ([iE, oZ])*
//Problem14.c
//(G (! ((oV & ! iE) & (F iE)) | (! oY U iE)));
#include <stdio.h> 
#include <assert.h>
#include <math.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! ((AP(output == 22) && ! AP(input == 5)) && (<> AP(input == 5))) || (! AP(output == 25) U AP(input == 5))));
