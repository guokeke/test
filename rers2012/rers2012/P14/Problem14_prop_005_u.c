//#Unsafe
//
//Formula: (false R (! (oY & ! iB) | (! oZ WU iB)))
//"output Z does never occur after output Y until input B"
//Formula is not satisfied! An error path is
//[iE, oV, iF, oY, iA, oU, iB, oZ, iE, oY, iD, oV, iE, oZ] ([iE, oZ])*
//Problem14.c
//(G (! (oY & ! iB) | (! oZ WU iB)));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(output == 25) && ! AP(input == 2)) || (! AP(output == 26) WU AP(input == 2))));
