//#Unsafe
//
//Formula: (false R (! (iF & ! iC) | (! oU WU iC)))
//"output U does never occur after input F until input C"
//Formula is not satisfied! An error path is
//[iD, oU, iF, oV, iA, oZ, iA, oU] ([iA, oU])*
//Problem16.c
//(G (! (iF & ! iC) | (! oU WU iC)));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(input == 6) && ! AP(input == 3)) || (! AP(output == 21) WU AP(input == 3))));
