//#Unsafe
//
//Formula: (false R (! ((iA & ! oZ) & (true U oZ)) | (! oU U oZ)))
//"output U does never occur between input A and output Z"
//Formula is not satisfied! An error path is
//[iA, oV, iE, oU, iB, oZ, iF, oY] ([iF, oV, iE, oY])*
//Problem16.c
//(G (! ((iA & ! oZ) & (F oZ)) | (! oU U oZ)));
#include <stdio.h> 
#include <assert.h>
#include <math.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! ((AP(input == 1) && ! AP(output == 26)) && (<> AP(output == 26))) || (! AP(output == 21) U AP(output == 26))));
