//#Unsafe
//
//Formula: (false R (! (oZ & ! iA) | (! iA U (oY & ! iA))))
//"output Y occurs after output Z until input A"
//Formula is not satisfied! An error path is
//[iD, oU, iF, oV, iA, oZ] ([iA, oU])*
//Problem16.c
//(G (! (oZ & ! iA) | (! iA U (oY & ! iA))));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(output == 26) && ! AP(input == 1)) || (! AP(input == 1) U (AP(output == 25) && ! AP(input == 1)))));
