//#Unsafe
//
//Formula: (false R (! ((iA & ! oX) & (true U oX)) | (! oV U oX)))
//"output V does never occur between input A and output X"
//Formula is not satisfied! An error path is
//[iD, oU, iF, oV, iA, oZ, iF, oV, iE, oU, iE, oX] ([iA, oU])*
//Problem16.c
//(G (! ((iA & ! oX) & (F oX)) | (! oV U oX)));
#include <stdio.h> 
#include <assert.h>
#include <math.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! ((AP(input == 1) && ! AP(output == 24)) && (<> AP(output == 24))) || (! AP(output == 22) U AP(output == 24))));
