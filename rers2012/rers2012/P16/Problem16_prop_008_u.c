//#Unsafe
//
//Formula: (false R (! ((oX & ! iF) & (true U iF)) | (! oV U iF)))
//"output V does never occur between output X and input F"
//Formula is not satisfied! An error path is
//[iD, oU, iF, oV, iF, oX, iC, oZ, iA, oV, iF] ([iA, oV, iF])*
//Problem16.c
//(G (! ((oX & ! iF) & (F iF)) | (! oV U iF)));
#include <stdio.h> 
#include <assert.h>
#include <math.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! ((AP(output == 24) && ! AP(input == 6)) && (<> AP(input == 6))) || (! AP(output == 22) U AP(input == 6))));
