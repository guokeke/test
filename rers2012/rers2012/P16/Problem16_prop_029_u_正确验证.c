//#Unsafe
//
//Formula: (false R (! ((iC & ! iE) & (true U iE)) | (! oV U iE)))
//"output V does never occur between input C and input E"
//Formula is not satisfied! An error path is
//[iA, oV, iF, oU, iC, oV, iE, oZ, iF, oZ] ([iB, oZ, iA, oY, iC, oV, iF])*
//Problem16.c
//(G (! ((iC & ! iE) & (F iE)) | (! oV U iE)));
#include <stdio.h> 
#include <assert.h>
#include <math.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! ((AP(input == 3) && ! AP(input == 5)) && (<> AP(input == 5))) || (! AP(output == 22) U AP(input == 5))));
