//#Unsafe
//
//Formula: (false R (! (iD & ! iF) | (! iF U (oW & ! iF))))
//"output W occurs after input D until input F"
//Formula is not satisfied! An error path is
//[iD, oV] ([iD, oZ, iA, oZ, iE])*
//Problem15.c
//(G (! (iD & ! iF) | (! iF U (oW & ! iF))));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(input == 4) && ! AP(input == 6)) || (! AP(input == 6) U (AP(output == 23) && ! AP(input == 6)))));
