//#Unsafe
//
//Formula: (false R (! (oZ & ! iA) | (! iA U (oU & ! iA))))
//"output U occurs after output Z until input A"
//Formula is not satisfied! An error path is
//[iD, oV, iD, oZ] ([iD, oV, iE])*
//Problem15.c
//(G (! (oZ & ! iA) | (! iA U (oU & ! iA))));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(output == 26) && ! AP(input == 1)) || (! AP(input == 1) U (AP(output == 21) && ! AP(input == 1)))));
