//#Unsafe
//
//Formula: (false R (! (iA & ! iD) | (! oY WU iD)))
//"output Y does never occur after input A until input D"
//Formula is not satisfied! An error path is
//[iB, oY, iD, oU, iE, oV, iB, oZ, iA, oY] ([iE, oU, iC, oV, iD, oV, iB, oZ, iA, oY])*
//Problem18.c
//(G (! (iA & ! iD) | (! oY WU iD)));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(input == 1) && ! AP(input == 4)) || (! AP(output == 25) WU AP(input == 4))));
