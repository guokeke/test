//#Unsafe
//
//Formula: (false R (! ((iA & ! iF) & (true U iF)) | (! oV U iF)))
//"output V does never occur between input A and input F"
//Formula is not satisfied! An error path is
//[iB, oY, iD, oU, iC, oZ, iF, oY, iF, oY, iC, oU, iA, oZ, iD, oY, iB, oV, iF] ([oU, iD, oY, iF])*
//Problem18.c
//(G (! ((iA & ! iF) & (F iF)) | (! oV U iF)));
#include <stdio.h> 
#include <assert.h>
#include <math.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! ((AP(input == 1) && ! AP(input == 6)) && (<> AP(input == 6))) || (! AP(output == 22) U AP(input == 6))));
