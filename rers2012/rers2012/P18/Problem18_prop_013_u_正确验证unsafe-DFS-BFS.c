//#Unsafe
//
//Formula: (false R (! (iE & ! iA) | (! iA U (oU & ! iA))))
//"output U occurs after input E until input A"
//Formula is not satisfied! An error path is
//[iB, oY, iD, oU, iE, oV] ([iB, oZ, iE, oV, iC, oZ, iE, oY, iC, iE, oZ, iB, iD, oV])*
//Problem18.c
//(G (! (iE & ! iA) | (! iA U (oU & ! iA))));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(input == 5) && ! AP(input == 1)) || (! AP(input == 1) U (AP(output == 21) && ! AP(input == 1)))));
