//#Unsafe
//
//Formula: (false R (! (oV & ! iC) | (! iC WU (oU & ! iC))))
//"output U occurs between output V and input C"
//Formula is not satisfied! An error path is
//[iB, oY, iE, oU, iD, oV, iC] ([oZ, iC, oV, iE, oV, iD, iC])*
//Problem18.c
//(G (! (oV & ! iC) | (! iC WU (oU & ! iC))));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(output == 22) && ! AP(input == 3)) || (! AP(input == 3) WU (AP(output == 21) && ! AP(input == 3)))));
