//#Unsafe
//
//Formula: (false R (! ((iC & ! iB) & (true U iB)) | (! oV U iB)))
//"output V does never occur between input C and input B"
//Formula is not satisfied! An error path is
//[iB, oY, iD, oU, iC, oZ, iC, oV, iD, oY, iB] ([oU, iA, oY, iC, oV, iD, oY, iB])*
//Problem18.c
//(G (! ((iC & ! iB) & (F iB)) | (! oV U iB)));
#include <stdio.h> 
#include <assert.h>
#include <math.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! ((AP(input == 3) && ! AP(input == 2)) && (<> AP(input == 2))) || (! AP(output == 22) U AP(input == 2))));
