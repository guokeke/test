//#Unsafe
//
//Formula: (false R (! (iA & ! iE) | (! iE U (oU & ! iE))))
//"output U occurs after input A until input E"
//Formula is not satisfied! An error path is
//[iB, oY, iD, oU, iC, oZ, iA] ([oZ, iB, oY, iF, oZ, iA, oY, iA])*
//Problem18.c
//(G (! (iA & ! iE) | (! iE U (oU & ! iE))));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(input == 1) && ! AP(input == 5)) || (! AP(input == 5) U (AP(output == 21) && ! AP(input == 5)))));
