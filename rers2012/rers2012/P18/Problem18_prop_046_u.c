//#Unsafe
//
//Formula: (false R (! (oY & ! iE) | (! iE WU (oU & ! iE))))
//"output U occurs between output Y and input E"
//Formula is not satisfied! An error path is
//[iB, oY, iE, oU] ([iD, oV, iD, oY, iD, oY, iE, oZ, iD])*
//Problem18.c
//(G (! (oY & ! iE) | (! iE WU (oU & ! iE))));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(output == 25) && ! AP(input == 5)) || (! AP(input == 5) WU (AP(output == 21) && ! AP(input == 5)))));
