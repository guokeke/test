//#Unsafe
//
//Formula: (false R (! (iE & ! iB) | (! iB WU (oV & ! iB))))
//"output V occurs between input E and input B"
//Formula is not satisfied! An error path is
//[iF, oW, iA, oY, iB, oU, iE, oU, iB] ([oW, iD, oY, iB, oU, iE, oU, iB])*
//Problem19.c
//(G (! (iE & ! iB) | (! iB WU (oV & ! iB))));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(input == 5) && ! AP(input == 2)) || (! AP(input == 2) WU (AP(output == 22) && ! AP(input == 2)))));
