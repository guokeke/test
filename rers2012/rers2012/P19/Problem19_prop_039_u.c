//#Unsafe
//
//Formula: (false R (! (iC & ! iA) | (! oW WU iA)))
//"output W does never occur after input C until input A"
//Formula is not satisfied! An error path is
//[iA, oW, iF, oY, iC, oW] ([iD, oU, iB, oU, iA, oW, iF, oU])*
//Problem19.c
//(G (! (iC & ! iA) | (! oW WU iA)));
#include <stdio.h> 
#include <assert.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! (AP(input == 3) && ! AP(input == 1)) || (! AP(output == 23) WU AP(input == 1))));
