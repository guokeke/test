//#Unsafe
//
//Formula: (false R (! ((iA & ! iE) & (true U iE)) | (! oU U iE)))
//"output U does never occur between input A and input E"
//Formula is not satisfied! An error path is
//[iF, oW, iA, oY, iB, oU, iE] ([oU, iF, oY, iB, oW, iF, iE])*
//Problem19.c
//(G (! ((iA & ! iE) & (F iE)) | (! oU U iE)));
#include <stdio.h> 
#include <assert.h>
#include <math.h>
...
int input, output;
	//@ ltl invariant positive: ([] (! ((AP(input == 1) && ! AP(input == 5)) && (<> AP(input == 5))) || (! AP(output == 21) U AP(input == 5))));
